...
from mmvpr.structures import DataSample, MultiTaskDataSample
...
@TRANSFORMS.register_module()
class PackVPRInputs(BaseTransform):
    """Pack the inputs data.
    ...
    **Added Keys:**
    ...
    DEFAULT_META_KEYS = ('sample_idx', 'img_path', 'ori_shape', 'img_shape',
                         'scale_factor', 'flip', 'flip_direction', 'img_paths')
    ...
    def transform(self, results: dict) -> dict:
        ...
        if self.input_key in results:
            input_ = results[self.input_key]
            packed_results['inputs'] = torch.stack(self.format_input(input_), dim=0)

        data_sample = DataSample()

        # Set default keys
        if 'place_id' in results:
            data_sample.set_place_id(results['place_id'])
