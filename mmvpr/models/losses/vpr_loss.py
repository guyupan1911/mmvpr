...
class VPRLoss(torch.nn.Module):
    ...
    def forward(self, embeddings: torch.Tensor, labels: torch.Tensor):
        ...
        if self.miner is not None:
            ...
            batch_accuracy = torch.tensor(1 - (unique_labels_mined / nb_samples))  # batch accuracy
        else:
            loss_value = self.loss_fn(embeddings, labels)
        return {'loss': loss_value, 'batch_accuracy': batch_accuracy}
