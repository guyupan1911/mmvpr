...
from mmvpr.registry import RUNNERS, MODELS
...
def visualization_gsv(data_samples: List[DataSample]):
    rows = len(data_samples)
    cols = len(data_samples[0].img_paths)

    fig, axes = plt.subplots(rows, cols, figsize=(cols * 5, rows * 5))

    for i, place in enumerate(data_samples):
        for j, filename in enumerate(place.img_paths):
            image = Image.open(filename)
            axes[i, j].imshow(image)
    plt.show()

def visualization_pittsburg(data_samples: List[DataSample]):
    print(data_samples[0])
    rows = len(data_samples)
    cols = len(data_samples[0].gt)

    fig, axes = plt.subplots(rows, cols, figsize=(cols * 5, rows * 5))

    query_image = Image.open(data_samples[0].img_path)
